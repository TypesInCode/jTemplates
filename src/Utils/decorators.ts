...
function ScopeDecorator<T extends Component<any, any, any>, K extends string>(target: T, propertyKey: K, descriptor: PropertyDescriptor) {
    ...
    DestroyDecorator(target as T & Record<K, ObservableScope<any>>, `ScopeDecorator_${propertyKey}`);
    return {
        configurable: false,
        enumerable: true,
        get: function() {
            ...
            if(!scope)
                scope = this[`ScopeDecorator_${propertyKey}`] = new ObservableScope(descriptor.get.bind(this));
...
function ComputedDecorator<T extends Component<any, any, any>, K extends string>(storeConstructor: { new(init: any): StoreClass<any> }, target: T, propertyKey: K, descriptor: PropertyDescriptor) {
    ...
    DestroyDecorator(target as T & Record<K, ObservableScope<any>>, `ComputedDecorator_Scope_${propertyKey}`);
