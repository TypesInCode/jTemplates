...
class DataBinding extends Binding<{(c: any, i: number): BindingDefinitions<any, any>}> {
    ...
    protected Apply() {
        ...
        for(var x=0; x<value.length; x++) {
            ...
            var newTemplates = this.activeTemplateMap.get(newKey); // || (this.childrenFunction(value[x], x) as Array<any>);
            if(!newTemplates) {
            ...
            if(x >= this.activeKeys.length) {
                ...
                newTemplates.forEach(t => {
                    // t.AttachBefore(this.BoundTo, nextTemplate);
            ...
            // previousTemplate = newTemplates[newTemplates.length - 1];
        }
