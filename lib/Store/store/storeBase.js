...
const storeQueryAsync_1 = require("./storeQueryAsync");
const storeQuerySync_1 = require("./storeQuerySync");
class AbstractStore {
    Action(action) {
...
class StoreBase extends AbstractStore {
    ...
    QuerySync(id, defaultValue, queryFunc) {
        if (this.queryCache.has(id))
            return this.queryCache.get(id);
        var query = new storeQuerySync_1.StoreQuerySync(this, defaultValue, queryFunc);
        var destroy = () => {
    ...
    Query(id, defaultValue, queryFunc) {
        if (this.queryCache.has(id))
            return this.queryCache.get(id);
        var query = new storeQueryAsync_1.StoreQueryAsync(this, defaultValue, queryFunc);
        var destroy = () => {
            this.queryCache.delete(id);
            query.removeListener("destroy", destroy);
        };
